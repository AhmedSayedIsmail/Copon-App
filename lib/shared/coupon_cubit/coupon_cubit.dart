...
class CouponCubit extends Cubit<CouponStates> {
  ...
  void loginWithEmailAndPassword(
    ...
    try {
      ...
      auth
          .signInWithEmailAndPassword(email: email, password: password)
          .then((value) {
        if (isAdmin && adminsuidList.contains(userCredential!.user!.uid)) {
          ...
          navigateAndFinish(context, const HomeScreen());
            ...
            usersuidList.contains(userCredential!.user!.uid)) {
          emit(LogInSuccessState());
          Fluttertoast.showToast(
            msg: kLoginSuccessfully,
            toastLength: Toast.LENGTH_SHORT,
          ...
          emit(YouWriteEmailInWrongPlace());
          Fluttertoast.showToast(
            msg:"This Email Written In Wrong Place",
            toastLength: Toast.LENGTH_SHORT,
            gravity: ToastGravity.CENTER,
          );
        }
  ...
  //This for pickImage in AddCouponScreen
...
//Here We will store coupondata in Firestore Store Image Path
//in fireStorage In AddCouponScreen
  ...
  //this to get Data of the specific Code in EditScreen
  ...
  //this admin to choose image from gallery in EditScreen
  ...
  //this to display image in Image Place in EditScreen
  ...
  //this to check that cuponcode is exist or not
  void checkOnCouponCode(
    ...
    if (snapShot.exists && cuponcode == codenumber) {
      // Document already exists and with same codeNumber
      updateData(
          companyname: companyname,
  ...
  void updateData(
      {required companyname, required companydesc, required cuponcode}) async {
    //this to check type of image to filter it
    if (companyimage is File) {
      print("File");
      //add company image in database and delete oldcompony image
      deleteAndupdate(
      ...
      print("Not File");
      //this function to add updates that happened
      //in the texts only and no changes on the image
      updateTexts(
          companyimage: companyimage,
          companyname: companyname,
          companydesc: companydesc,
  ...
  void deleteAndupdate(
    ...
    // print("OLDIMAGE: $oldImage");//this is CouponImages/image_picker6966735042236324278.jpg
    await firebase_storage.FirebaseStorage.instance
        .ref()
        .child(oldImage)
        .delete();
  ...
  void deleteOldCouponAndaddNewCoupon(
    ...
    //this to Delete OldData From FireStorage And FireCloudDataBase
    await firebase_storage.FirebaseStorage.instance
        .ref()
...
//This To Add New Data In FireStorage And FireCloudDataBase
    await firebase_storage.FirebaseStorage.instance
        .ref()
  ...
  void getUidsOfUsers() async {
    ...
    await FirebaseFirestore.instance
        ...
        .then((value) {
      value.docs.forEach((element) {
        usersuidList.add(element.data()['uid']);
      });
    });
    print(usersuidList);
  }

  void getUidsOfAdmins() async {
    adminsuidList = [];
    await FirebaseFirestore.instance
        .collection(kadminCollection)
        .get()
        .then((value) {
      value.docs.forEach((element) {
        adminsuidList.add(element.data()['uid']);
      });
    });
    print(adminsuidList);
  }
